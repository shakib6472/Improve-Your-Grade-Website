...
class Edubin_Elementor_Widget_Contact_Form_Seven extends Widget_Base {
    ...
    protected function register_controls()
    {
        ...
        $this->add_responsive_control(
            'edubin_box_border_radius',
            [
                'label' => esc_html__( 'Border Radius', 'edubin-core' ),
                'type' => Controls_Manager::DIMENSIONS,
                'selectors' => [
                    '{{WRAPPER}} .edubin-form-wrapper' => 'border-radius: {{TOP}}px {{RIGHT}}px {{BOTTOM}}px {{LEFT}}px;',
                ],
            ]
        );
        $this->end_controls_section();
    ...
    protected function render($instance = [])
    {
        ...
        // Render Title
        $title = ($settings['show_form_title']) ? ('<div class="wpcf7-title">'.$settings['heading_text'].'</div>') : ('');
        $description = ($settings['show_form_description']) ? (' <div class="wpcf7-description">'.$settings['desc_text'].'</div>') : ('');

        echo '<div '.$this->get_render_attribute_string('edubin_form_area_attr').' >';
            if($settings['show_form_title'] || $settings['show_form_description']){
                echo '<div class="wpcf7-head-container">';
                    echo $title;
                    echo $description;
                echo '</div>';
            };
