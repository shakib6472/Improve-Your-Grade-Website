...
class Course_Filter extends Widget_Base {
	...
	public function get_keywords() {
		return [ 'edubin', 'lp', 'courses', 'course', 'lms', 'learnpress','filter', 'Course filter', 'sidebar filter', 'filter sidebar', 'course category', 'course tag', 'course instructor', 'instructor', 'course level' ];
	}
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default'       => [ 'category', 'lp_level', 'lp_price', 'languages', 'instructor' ],
                'multiple'      => true,
                'options'       => $filter_by                   
            ]
        );
    ...
    protected function render() {
        ...
        $settings['course_cpt'] = LP_COURSE_CPT; 
        ...
        $search_placeholder_text = $category_label_text = $price_label_text = $level_label_text = '';
        $category_number = 0;
        ...
        if ( in_array( 'category', $settings['filter_options'] ) ) :
            ...
            $settings['course_category'] = 'course_category';
        ...
        if ( in_array( 'languages', $settings['filter_options'] ) ) :
            ...
            $settings['course_language'] = 'lp_course_language';
        ...
        if ( in_array( 'lp_price', $settings['filter_options'] ) ) :
            $price_label_text = $settings['price_label_text'];
            $settings['price_label_text'] = $price_label_text;
        endif;
        
        if ( in_array( 'lp_level', $settings['filter_options'] ) ) :
        ...
        $settings['orderby_types'] = apply_filters( 'edubin_courses_orderby', array(
            ...
            'lp_popular'      => __( 'Popular', 'edubin-core' ),
            'lp_featured'     => __( 'featured', 'edubin-core' ),
        ) );
            ...
            echo '<div ' . $this->get_render_attribute_string( 'wrapper' ) . '>';
                ...
                echo '<div class="edubin-col-lg-9 filter-course-column">';
                    echo '<div class="filtered-courses">';
                        if ( ( isset( $_GET['course_price'] ) && ( 'free' === $_GET['course_price'] ) ) && ! isset( $_GET['reset'] ) && in_array( 'lp_price', $settings['filter_options'] ) ) :
                        ...
                        if ( ( isset( $_GET['course_price'] ) && ( 'paid' === $_GET['course_price'] ) ) && ! isset( $_GET['reset'] ) && in_array( 'lp_price', $settings['filter_options'] ) ) :
                            $args['meta_query'][] = array(
                                'key'     => '_lp_price',
                                'value'   => '0',
                                'compare' => '>'
                        ...
                        if ( ( ! empty( $filtered_level ) && ! in_array( 'all_levels', $filtered_level ) ) && in_array( 'lp_level', $settings['filter_options'] ) ) :
                            $args['meta_query'][] = array(
                                'key'     => '_lp_level',
                        ...
                        $args = apply_filters( 'edubin_lp_course_filter_args', $args );
                        ...
                        if ( $query->have_posts() ) :
                           ...
                           if ( $settings['show_author_name'] ) :
                            ...
                            if ( $settings['show_wishlist_list'] ) :
                                $layout_data['show_wishlist_list'] = $settings['show_wishlist_list'];
                            endif;
                                                    
                            if ( $settings['show_excerpt_list'] ) :
                                 $layout_data['show_excerpt_list'] = $settings['show_excerpt_list'];
                            endif;
        
                            if ( $settings['excerpt_length_list'] ) :
                                 $layout_data['excerpt_length_list'] = $settings['excerpt_length_list'];
                            endif;

                            if ( $settings['show_cat_list'] ) :
                                $layout_data['show_cat_list'] = $settings['show_cat_list'];
                            endif;

                            if ( $settings['show_lessons_list'] ) :
                                $layout_data['show_lessons_list'] = $settings['show_lessons_list'];
                            endif;

                            if ( $settings['show_lessons_text_list'] ) :
                                $layout_data['show_lessons_text_list'] = $settings['show_lessons_text_list'];
                            endif;

                            if ( $settings['show_enrolled_list'] ) :
                                $layout_data['show_enrolled_list'] = $settings['show_enrolled_list'];
                            endif;

                            if ( $settings['show_enrolled_text_list'] ) :
                                $layout_data['show_enrolled_text_list'] = $settings['show_enrolled_text_list'];
                            endif;

                            if ( $settings['show_quiz_list'] ) :
                                $layout_data['show_quiz_list'] = $settings['show_quiz_list'];
                            endif;

                            if ( $settings['show_quiz_text_list'] ) :
                                $layout_data['show_quiz_text_list'] = $settings['show_quiz_text_list'];
                            endif;
                            if ( $settings['show_review_list'] ) :
                                $layout_data['show_review_list'] = $settings['show_review_list'];
                            endif;
                
                            if ( $settings['show_review_list_text'] ) :
                                $layout_data['show_review_list_text'] = $settings['show_review_list_text'];
                            endif;
                            ...
                            if ( 'list' !== $settings['content_type'] ) :
                                ...
                                echo '<div ' . $this->get_render_attribute_string( 'grid' ) . '>';
                                    while ( $query->have_posts() ) : $query->the_post();
                                        ...
                                        $layout_data['thumb_url'] = $thumb_url;
                                        echo '<div ' . $this->get_render_attribute_string( 'grid_single' ) . '>';

                                            $layout_data['style'] = $settings['grid_style'];

                                            if ( $settings['grid_excerpt_length'] ) :
                                                $layout_data['excerpt_length'] = $settings['grid_excerpt_length'];
                                            endif;
                                ...
                                echo '<div ' . $this->get_render_attribute_string( 'list' ) . '>';
                                    while ( $query->have_posts() ) : $query->the_post();
                                        ...
                                        echo '<div ' . $this->get_render_attribute_string( 'list_single' ) . '>';
                                            ...
                                            $layout_data['style'] = $settings['list_style'];
                                            if ( $settings['excerpt_length_list'] ) :
                                                $layout_data['excerpt_length'] = $settings['excerpt_length_list'];
                                            endif;           
