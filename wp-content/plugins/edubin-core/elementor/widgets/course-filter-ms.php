...
class Course_Filter extends Widget_Base {
    ...
    public function get_name() {
        return 'edubin-course-filter-ms';
    }

    public function get_title() {
        return __( 'Course Filter (MasterStudy)', 'edubin-core' );
    }
    ...
    public function get_keywords() {
        return [ 'edubin', 'masterstudy', 'courses', 'course', 'lms','filter', 'Course filter', 'sidebar filter', 'filter sidebar', 'course category', 'course tag', 'course instructor', 'instructor', 'course level' ];
    }
    ...
    protected function _register_controls() {
        $filter_by = apply_filters( 'edubin_course_filter_options', [
            ...
            'ms_status'  => __( 'Status', 'edubin-core' ),
            'ms_level'   => __( 'Level', 'edubin-core' ),
            'ms_price'   => __( 'Price', 'edubin-core' ),
            // 'ms_rating'  => __( 'rating', 'edubin-core' )
        ] );
        ...
        $this->add_control(
            ...
            [
                ...
                'default'       => ['search', 'category', 'instructor', 'ms_price'],
                'multiple'      => true,
                'options'       => $filter_by                   
            ]
        );
        ...
        $this->add_control(
            'show_enrolled',
            [
                'label' => esc_html__( 'Student Enrolled?', 'edubin-core' ),
                'type' => Controls_Manager::SWITCHER,
                'return_value' => 'yes',
                'default' => 'yes',
            ]
        ...
        $this->add_control(
            'show_enrolled_text',
            [
                'label' => esc_html__( 'Student Text?', 'edubin-core' ),
                'type' => Controls_Manager::SWITCHER,
                'return_value' => 'yes',
                'default' => 'yes',
                'condition' => [
       ...
       $this->add_control(
        ...
        $this->add_control(
            ...
            [
                ...
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'description'        => __( 'Number of categories to show. By default the value is 0, which shows all the categories.', 'edubin-core' ),
                'condition'          => [
                    'filter_options' => 'category'
                ]
            ]
        ...
        $this->add_control(
            ...
            [
                'type'               => Controls_Manager::TEXT,
                'label'              => __( 'Status Label Text', 'edubin-core' ),
                'default'            => __( 'Status', 'edubin-core' ),
                'condition'          => [
                    'filter_options' => 'ms_status'
                ]
            ]
        );
        ...
        //     [
        //         'type'               => Controls_Manager::TEXT,
        //         'label'              => __( 'Rating Label Text', 'edubin-core' ),
        //         'default'            => __( 'Rating', 'edubin-core' ),
        //         'condition'          => [
        //             'filter_options' => 'ms_rating'
        //         ]
        //     ]
        // );
    ...
    protected function render() {
        ...
        $settings['course_cpt'] = 'stm-courses'; 

        // Initialize filtered categories as empty arrays
        ...
        $filtered_status = ! empty( $_GET['course_status'] ) ? ( array ) $_GET['course_status'] : array();
        $filtered_status = array_map( 'sanitize_text_field', $filtered_status );
        ...
        $filtered_rating = ! empty( $_GET['rate'] ) ? $_GET['rate'] : '';
        ...
        // Handle the reset button for category, search value & course ordering
        if ( isset( $_GET['reset'] ) ) :
        ...
        if ( in_array( 'category', $settings['filter_options'] ) ) :
            ...
            $settings['course_category'] = 'stm_lms_course_taxonomy';
        ...
        //     $settings['course_tag'] = 'stm-courses';
        ...
        if ( in_array( 'languages', $settings['filter_options'] ) ) :
            ...
            $settings['course_language'] = 'ms_course_language';
        ...
        if ( in_array( 'ms_price', $settings['filter_options'] ) ) :
            $price_label_text = $settings['price_label_text'];
            $settings['price_label_text'] = $price_label_text;
        endif;

        if ( in_array( 'ms_status', $settings['filter_options'] ) ) :
            $status_label_text = $settings['status_label_text'];
            $settings['filtered_status'] = $filtered_status;
            $settings['status_label_text'] = $status_label_text;
        endif;

        if ( in_array( 'ms_level', $settings['filter_options'] ) ) :
        ...
        // if ( in_array( 'ms_rating', $settings['filter_options'] ) ) :
        //     $rating_label_text = $settings['rating_label_text'];
        //     $settings['filtered_rating'] = $filtered_rating;
        //     $settings['rating_label_text'] = $rating_label_text;
        // endif;
        ...
        $args = Filter::query( $filtered_category, null, $filtered_languages, $settings );
        ...
        if ( ! empty( $filtered_status ) && in_array( 'ms_status', $settings['filter_options'] ) ) :
        ...
        if ( ! empty( $filtered_level ) && in_array( 'ms_level', $settings['filter_options'] ) ) :
        ...
        // if ( ! empty( $filtered_rating ) && in_array( 'ms_rating', $settings['filter_options'] ) ) :
        //     $args['meta_query'][] = array(
        //         'key'     => 'course_mark_average',
        //         'value'   => floatval( $filtered_rating ),
        //         'compare' => '>='
        //     );
        // endif;
            ...
            echo '<div ' . $this->get_render_attribute_string( 'wrapper' ) . '>';
                ...
                echo '<div class="edubin-col-lg-9 filter-course-column">';
                    echo '<div class="filtered-courses">';
                        if ( isset( $_GET['course_price'] ) && ! isset( $_GET['reset'] ) && in_array( 'ms_price', $settings['filter_options'] ) ) :
                            if ( 'free' === $_GET['course_price'] ) :
                            ...
                            elseif ( 'subscription' === $_GET['course_price'] ) :
                        ...
                        $args = apply_filters( 'edubin_ms_course_filter_args', $args );
                                ...
                                echo '<div ' . $this->get_render_attribute_string( 'grid' ) . '>';
                                    while ( $query->have_posts() ) : $query->the_post();
                                        ...
                                        ?>
                                            ...
                                            <?php
                                                \STM_LMS_Templates::show_lms_template( 'tpl-part/course/el-layouts', compact( 'get_options' ) ); 
                                            echo '</div>';  
                                        echo '</div>';  
                                    endwhile;
                                    wp_reset_postdata();
                                    ...
                                    while ( $query->have_posts() ) : $query->the_post();
                                        ...
                                        echo '<div ' . $this->get_render_attribute_string( 'list_single' ) . '>';
                                            ...
                                            if ( $settings['show_review_list_text'] ) :
                                                $get_options['show_review_list_text'] = $settings['show_review_list_text'];
                                            endif;
