...
if ( ! class_exists( 'WPSC_CF_URL' ) ) :

	final class WPSC_CF_URL {
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 10 );
		...
		public static function get_tff_value( $slug, $cf = false ) {
			...
			$url = isset( $_POST[ $slug ] ) ? esc_url_raw( wp_unslash( $_POST[ $slug ] ) ) : ''; // phpcs:ignore
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					$url = WPSC_Functions::sanitize_url( sanitize_text_field( $request->get_param( $cf->slug ) ) );

					if ( in_array( $cf->field, array( 'ticket', 'agentonly' ) ) ) {

						$data[ $cf->slug ] = $cf->field == 'ticket' && $url ? $url : self::get_default_value( $cf );
						...
						if ( $url && $url != $existing_val ) {

							$customer->{$cf->slug} = $url;
							$customer->save();
		...
		public static function get_cf_input_val( $cf ) {
			...
			$url = isset( $_POST[ $cf->slug ] ) ? esc_url_raw( wp_unslash( $_POST[ $cf->slug ] ) ) : ''; // phpcs:ignore
			$url = filter_var( $url, FILTER_VALIDATE_URL );
			return $url ? $url : '';
		}
		...
		public static function set_edit_ticket_cf( $cf, $ticket ) {
			...
			$new = isset( $_POST[ $cf->slug ] ) ? esc_url_raw( wp_unslash( $_POST[ $cf->slug ] ) ) : ''; // phpcs:ignore
			$new = filter_var( $new, FILTER_VALIDATE_URL ) ? $new : '';
		...
		public static function set_rest_edit_ticket_cf( $ticket, $cf, $value ) {
			...
			$url = WPSC_Functions::sanitize_url( sanitize_text_field( $value ) );
			if ( $ticket->{$cf->slug} != $url ) {
				$ticket->{$cf->slug} = $url;
			}
		}
		...
		public static function print_edit_customer_info( $cf, $customer, $tff ) {
			...
			$val = $customer->id && $customer->{$cf->slug} ? htmlspecialchars( $customer->{$cf->slug} ) : '';
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				?>
				<div data-type="url" data-required="false" class="wpsc-input-group default_value">
					<div class="label-container">
						<label for="">
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="url" data-required="false" class="wpsc-input-group default_value">
					...
					<input type="text" name="default_value" value="<?php echo esc_attr( $default_val ); ?>" autocomplete="off">
				</div>
				<?php
			endif;
		...
		public static function set_cf_properties( $cf, $field_class ) {
			...
			// default value.
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) {
				$default_value     = isset( $_POST['default_value'] ) ? esc_url_raw( wp_unslash( $_POST['default_value'] ) ) : ''; // phpcs:ignore
				$default_value     = filter_var( $default_value, FILTER_VALIDATE_URL );
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_url', $ticket->{$cf->slug}, $cf, $ticket, $module );
		}
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			...
			?>
			<a class="wpsc-link" href="<?php echo esc_url_raw( $url ); ?>" target="_blank"><?php esc_attr_e( 'Click Here', 'supportcandy' ); ?></a>
			<?php
		}
		...
		public static function print_val( $cf, $val ) {
			...
			?>
			<a class="wpsc-link" href="<?php echo esc_url_raw( $val ); ?>" target="_blank"><?php echo esc_url_raw( $val ); ?></a>
			<?php
		}
		...
		public static function get_history_log_val( $cf, $val ) {
			...
			return $val ? esc_url_raw( $val ) : esc_attr__( 'None', 'supportcandy' );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to <a href="' . esc_attr( $url_value ) . '">' . esc_attr( $url_value ) . '</a>';
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			return $val ? '<a href="' . esc_url_raw( $val ) . '" target="__blank">' . esc_url_raw( $val ) . '</a>' : esc_attr__( 'None', 'supportcandy' );
		}
	}
endif;
