...
if ( ! class_exists( 'WPSC_DF_Status' ) ) :
	...
	final class WPSC_DF_Status {
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$new_status = new WPSC_Status( $body->new );
			if ( $new_status->id ) {
				$status_value = $new_status->name;
			} else {
				$status_value = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $status_value );
		}
	}
endif;
