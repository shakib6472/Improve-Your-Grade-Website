...
if ( ! class_exists( 'WPSC_CF_Checkbox' ) ) :
	...
	final class WPSC_CF_Checkbox {
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$checkbox_values = explode( '|', $body->new );
			$checkbox_val_names = array();
			foreach ( $checkbox_values as $new_val_id ) {
				$new_val = new WPSC_Option( $new_val_id );
				if ( $new_val->id ) {
					$checkbox_val_names[] = esc_attr( $new_val->name );
				}
			}
			if ( ! empty( $checkbox_val_names ) ) {
				$value_names = implode( ', ', $checkbox_val_names );
			} else {
				$value_names = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $value_names );
		}
