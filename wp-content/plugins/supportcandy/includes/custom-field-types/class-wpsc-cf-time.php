...
if ( ! class_exists( 'WPSC_CF_Time' ) ) :
	...
	final class WPSC_CF_Time {
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$new_val = DateTime::createFromFormat( 'H:i', $body->new );
			if ( $new_val ) {
				$log = $cf->time_format == 12 ? esc_attr( $new_val->format( 'h:i A' ) ) : esc_attr( $new_val->format( 'H:i' ) );
			} else {
				$log = 'None';
			}
		...
		public static function get_field_value( $cf, $val ) {
			...
			if ( $val ) {
				$date = DateTime::createFromFormat( 'H:i', $val );
				return $cf->time_format == 12 ? esc_attr( $date->format( 'h:i A' ) ) : esc_attr( $date->format( 'H:i' ) );
			} else {
				return esc_attr__( 'None', 'supportcandy' );
			}
		}
	}
endif;
