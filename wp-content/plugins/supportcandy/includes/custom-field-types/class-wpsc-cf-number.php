...
if ( ! class_exists( 'WPSC_CF_Number' ) ) :
	...
	final class WPSC_CF_Number {
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'number_type', $field_class::$allowed_properties ) ) :
				?>
				<div data-type="single-select" data-required="false" class="wpsc-input-group number_type">
					<div class="label-container">
						<label for="">
							<?php esc_attr_e( 'Select type', 'supportcandy' ); ?>
						</label>
					</div>
					<select name="number_type">
						<option value="integer"><?php esc_attr_e( 'Integer', 'supportcandy' ); ?></option>
						<option value="float"><?php esc_attr_e( 'Float', 'supportcandy' ); ?></option>
					</select>
				</div>
				<?php
			endif;
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'number_type', $field_class::$allowed_properties ) ) :
				...
				<div data-type="single-select" data-required="false" class="wpsc-input-group number_type">
					...
					<select name="number_type">
						<option <?php selected( $cf->number_type, 'integer' ); ?> value="integer"><?php esc_attr_e( 'Integer', 'supportcandy' ); ?></option>
						<option <?php selected( $cf->number_type, 'float' ); ?> value="float"><?php esc_attr_e( 'Float', 'supportcandy' ); ?></option>
					</select>
				</div>
				<?php
			endif;
		...
		public static function set_cf_properties( $cf, $field_class ) {
			...
			// number type.
			if ( in_array( 'number_type', $field_class::$allowed_properties ) ) {
				$cf->number_type = isset( $_POST['number_type'] ) ? sanitize_text_field( wp_unslash( $_POST['number_type'] ) ) : 'integer'; // phpcs:ignore
			}
		...
		public static function print_val( $cf, $val ) {
			...
			if ( $cf->number_type == 'float' ) {
				echo is_numeric( $val ) ? esc_attr( $val ) : esc_attr__( 'None', 'supportcandy' );
			} else {
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $number );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			if ( $cf->number_type == 'float' ) {
				return is_numeric( $val ) ? esc_attr( $val ) : esc_attr__( 'None', 'supportcandy' );
			} else {
				return is_numeric( $val ) ? intval( $val ) : esc_attr__( 'None', 'supportcandy' );
			}
		}
	}
endif;
