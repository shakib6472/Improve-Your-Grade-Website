...
if ( ! class_exists( 'WPSC_CF_Multi_Select' ) ) :

	final class WPSC_CF_Multi_Select {
		 ...
		 * @var string
		 */
		public static $slug = 'cf_multi_select';
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Dropdown (multi-select)', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		...
		public static function print_tff( $cf, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>[]" onchange="wpsc_check_tff_visibility()" multiple>
					<?php
					foreach ( $options as $option ) {
						$selected = in_array( $option->id, $val ) ? 'selected="selected"' : ''
						?>
						<option <?php echo esc_attr( $selected ); ?> value="<?php echo esc_attr( $option->id ); ?>"><?php echo esc_attr( $option->name ); ?></option>
						<?php
					}
		...
		public static function print_cf_input( $cf, $value = array() ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>[]" multiple>
					<?php
					$val = array_map( fn( $option )=>$option->id, $value );
					foreach ( $options as $option ) {
						$selected = $val && in_array( $option->id, $val ) ? 'selected' : ''
						?>
						<option <?php echo esc_attr( $selected ); ?> value="<?php echo esc_attr( $option->id ); ?>"><?php echo esc_attr( $option->name ); ?></option>
						<?php
					}
		...
		public static function print_edit_ticket_cf( $cf, $ticket ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>[]" multiple>
					<?php
					$val = array_map( fn( $option )=>$option->id, $ticket->{$cf->slug} );
					foreach ( $options as $option ) {
						$selected = $val && in_array( $option->id, $val ) ? 'selected' : ''
						?>
						<option <?php echo esc_attr( $selected ); ?> value="<?php echo esc_attr( $option->id ); ?>"><?php echo esc_attr( $option->name ); ?></option>
		...
		public static function print_edit_customer_info( $cf, $customer, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>[]" multiple>
					<?php
					foreach ( $options as $option ) {
						$selected = in_array( $option->id, $val ) ? 'selected' : '';
						?>
						<option <?php echo esc_attr( $selected ); ?> value="<?php echo esc_attr( $option->id ); ?>"><?php echo esc_attr( $option->name ); ?></option>
						<?php
					}
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_multi_select', $value, $cf, $ticket, $module );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$ms_values = explode( '|', $body->new );

			foreach ( $ms_values as $ms_value ) {
				$new_ms = new WPSC_Option( $ms_value );
				if ( $new_ms->id ) {
					$new_ms_values[] = esc_attr( $new_ms->name );
				}
			}
			if ( ! empty( $new_ms_values ) ) {
				$ms_names = implode( ', ', $new_ms_values );
			} else {
				$ms_names = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $ms_names );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$option_names = array_map( fn( $option ) => $option->name, $options );
			return $option_names ? esc_attr( implode( ', ', $option_names ) ) : esc_attr__( 'None', 'supportcandy' );
		}
	}
endif;

WPSC_CF_Multi_Select::init();
