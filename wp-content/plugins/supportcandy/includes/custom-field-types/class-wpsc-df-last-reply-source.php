...
if ( ! class_exists( 'WPSC_DF_Last_Reply_Source' ) ) :

	final class WPSC_DF_Last_Reply_Source {
		 ...
		 * @var string
		 */
		public static $slug = 'df_last_reply_source';
		...
		public static function init() {
			...
			// source list.
			add_action( 'init', array( __CLASS__, 'load_last_reply_sources' ) );
		}
		...
		/**
		 * Load last reply source list
		 *
		 * @return void
		 */
		public static function load_last_reply_sources() {

			self::$sources = apply_filters(
				'wpsc_last_reply_source_list',
				array(
					'browser'  => __( 'Browser', 'supportcandy' ),
					'rest-api' => 'REST API',
				)
			);
		}
			...
			<div class="item conditional operand single">
				<select class="operand_val_1 <?php echo esc_attr( $unique_id ); ?>" <?php echo $is_multiple ? 'multiple' : ''; ?>>
					...
					foreach ( self::$sources as $key => $label ) {
						$selected = '';
						if (
							isset( $filter['operand_val_1'] ) &&
							(
								( $is_multiple && in_array( $key, $filter['operand_val_1'] ) ) ||
								( ! $is_multiple && $key == $filter['operand_val_1'] )
							)
						) {
							$selected = 'selected';
						}
		...
		public static function set_create_ticket_data( $data, $custom_fields, $is_my_profile ) {
			...
			$data['last_reply_source'] = 'browser';
			return $data;
		}
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			$data['last_reply_source'] = 'rest-api';
			return $data;
		}
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			$value = $ticket->{$cf->slug} && isset( self::$sources[ $ticket->{$cf->slug} ] ) ? self::$sources[ $ticket->{$cf->slug} ] : '';
			return apply_filters( 'wpsc_ticket_field_val_last_reply_source', $value, $cf, $ticket, $module );
		}
		...
		public static function print_tl_ticket_field_val( $cf, $ticket ) {
			...
			echo $ticket->{$cf->slug} && isset( self::$sources[ $ticket->{$cf->slug} ] ) ? esc_attr( self::$sources[ $ticket->{$cf->slug} ] ) : '';
		}
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			...
			echo $ticket->{$cf->slug} && isset( self::$sources[ $ticket->{$cf->slug} ] ) ? esc_attr( self::$sources[ $ticket->{$cf->slug} ] ) : esc_attr__( 'Not Applicable', 'supportcandy' );
		}
	}
endif;

WPSC_DF_Last_Reply_Source::init();
