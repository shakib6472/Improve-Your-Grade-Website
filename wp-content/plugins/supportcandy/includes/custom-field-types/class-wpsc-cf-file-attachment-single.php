...
if ( ! class_exists( 'WPSC_CF_File_Attachment_Single' ) ) :
	...
	final class WPSC_CF_File_Attachment_Single {
		...
		public static function get_duplicate_ticket_data( $cf, $ticket ) {
			...
			return $ticket->{$cf->slug} ? WPSC_Individual_Ticket::get_duplicate_attachment_id( $ticket->{$cf->slug}->id ) : '';
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			if ( $body->new ) {
				$attachment = new WPSC_Attachment( $body->new );
				$url = site_url( '/' ) . '?wpsc_attachment=' . $attachment->id;
				$link = '<a class="wpsc-link" href=" ' . esc_url( $url ) . ' " target="_blank">' . $attachment->name . '</a>';
			} else {
				$link = 'None';
			}
