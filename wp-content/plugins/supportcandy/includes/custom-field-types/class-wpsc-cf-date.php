...
if ( ! class_exists( 'WPSC_CF_Date' ) ) :

	final class WPSC_CF_Date {
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 6 );
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Date', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		...
		public static function get_operands( $operator, $cf, $filter = array() ) {
			...
			if ( $is_between ) {
				...
				<div class="item conditional operand">
					<input type="text" class="operand_val_2 <?php echo esc_attr( $unique_id ); ?>" placeholder="<?php esc_attr_e( 'To date', 'supportcandy' ); ?>" value="<?php echo esc_attr( $operand_val_2 ); ?>">
				</div>
				<script>
		...
		public static function is_valid( $condition, $cf, $value ) {
			...
			$operand_1 = new DateTime( $condition['operand_val_1'] . ' 00:00:00' );
			...
			switch ( $condition['operator'] ) {
				...
				case 'BETWEEN':
					$operand_2 = new DateTime( $condition['operand_val_2'] . ' 00:00:00' );
		...
		public static function print_tff( $cf, $tff ) {
			...
			if ( $cf->field == 'ticket' ) {
				...
				$val = $val ? new DateTime( $val . ' 00:00:00' ) : '';
			} else {
			...
			ob_start();
			?>
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
		...
		public static function get_tff_value( $slug, $cf = false ) {
			...
			$date = $date && preg_match( '/\d{4}-\d{2}-\d{2}/', $date ) ? $date : '';
			return $date ? new DateTime( $date . ' 00:00:00' ) : false;
		}
		...
		public static function get_default_value( $cf ) {
			...
			return $cf->default_value ? $cf->default_value[0] . ' 00:00:00' : '';
		}
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					if ( $cf->field == 'ticket' ) {
						...
						$prev_val = $customer->{$cf->slug} && is_object( $customer->{$cf->slug} ) ? $customer->{$cf->slug}->format( 'Y-m-d' ) : '';
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					$date = WPSC_Functions::sanitize_date( sanitize_text_field( $request->get_param( $cf->slug ) ) );
		...
		public static function print_cf_input( $cf, $date = '' ) {
			...
			if ( $date && ! is_object( $date ) && $date != '0000-00-00 00:00:00' ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<div class="wpsc-input-close-group">
					<input 
						...
						placeholder="<?php echo esc_attr( $cf->placeholder_text ); ?>" 
						maxlength="<?php echo $cf->char_limit ? intval( $cf->char_limit ) : ''; ?>" 
						value="<?php echo $date && is_object( $date ) ? esc_attr( $date->format( 'Y-m-d' ) ) : ''; ?>"
						autocomplete="off"/>
					<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
				</div>
				<script>
		...
		public static function get_cf_input_val( $cf ) {
			...
			return $date && preg_match( '/\d{4}-\d{2}-\d{2}/', $date ) ? $date . ' 00:00:00' : '';
		}
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<div class="wpsc-input-close-group">
					<input 
						...
						placeholder="<?php echo esc_attr( $cf->placeholder_text ); ?>" 
						maxlength="<?php echo $cf->char_limit ? intval( $cf->char_limit ) : ''; ?>" 
						value="<?php echo $ticket->{$cf->slug} && is_object( $ticket->{$cf->slug} ) ? esc_attr( $ticket->{$cf->slug}->format( 'Y-m-d' ) ) : ''; ?>"
						autocomplete="off"/>
					<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
				</div>
				<script>
		...
		public static function set_rest_edit_ticket_cf( $ticket, $cf, $value ) {
			...
			$date = WPSC_Functions::sanitize_date( sanitize_text_field( $value ) );
			$new = $date ? $date : '';
			$prev = is_object( $ticket->{$cf->slug} ) ? $ticket->{$cf->slug}->format( 'Y-m-d H:i:s' ) : '';
			if ( $prev != $new ) {
				$ticket->{$cf->slug} = $new;
			}
		}
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="textfield" data-required="false" class="wpsc-input-group default_value">
					...
					<div class="wpsc-input-close-group">
						<input type="text" id="wpsc-default-val-date" name="default_value" />
						<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
					</div>
					<script>jQuery("#wpsc-default-val-date").flatpickr({
						locale: "<?php echo esc_attr( WPSC_Functions::get_locale_iso() ); ?>",
					});</script>
				</div>
				<?php
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="textfield" data-required="false" class="wpsc-input-group default_value">
					...
					<div class="wpsc-input-close-group">
						<input type="text" id="wpsc-default-val-date" name="default_value" value="<?php echo esc_attr( $default_val ); ?>" autocomplete="off"/>
						<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
					</div>
					<script>
						jQuery("#wpsc-default-val-date").flatpickr({
							locale: "<?php echo esc_attr( WPSC_Functions::get_locale_iso() ); ?>",
						});
					</script>
				</div>
			...
			<div data-type="date-select" data-required="false" class="wpsc-input-group wpsc-date-range-option" style="<?php echo $cf->date_range != 'range' ? 'display:none;' : ''; ?>">
				<div class="label-container">
					<label for=""><?php esc_attr_e( 'Date in between', 'supportcandy' ); ?></label>
				</div>
				<div class="wpsc-date-range">
					<div class="wpsc-input-close-group">
						<input type="text" id="from-date-range" name="from-date-range" value="<?php echo $cf->start_range && is_object( $cf->start_range ) ? esc_attr( $cf->start_range->format( 'Y-m-d' ) ) : ''; ?>" autocomplete="off"/>
						<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
					</div>
					<div style="margin:5px;"></div>
					<div class="wpsc-input-close-group">
						<input type="text" id="to-date-range" name="to-date-range" value="<?php echo $cf->end_range && is_object( $cf->end_range ) ? esc_attr( $cf->end_range->format( 'Y-m-d' ) ) : ''; ?>" autocomplete="off"/>
						<span onclick="wpsc_clear_date(this);"><?php WPSC_Icons::get( 'times' ); ?></span>
					</div>
				</div>
				<script>
		...
		public static function set_cf_properties( $cf, $field_class ) {
			...
			// date range in between.
			$cf->start_range = isset( $_POST['from-date-range'] ) ? new DateTime( sanitize_text_field( wp_unslash( $_POST['from-date-range'] ) ) ) : ''; // phpcs:ignore
			$cf->end_range   = isset( $_POST['to-date-range'] ) && $cf->start_range ? new DateTime( sanitize_text_field( wp_unslash( $_POST['to-date-range'] ) ) ) : ''; //phpcs:ignore
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_date', $value, $cf, $ticket, $module );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$new_val = new DateTime( $body->new );
			if ( $body->new ) {
				$log = date_i18n( $format, $new_val->getTimestamp() );
			} else {
				$log = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $log );
		}
