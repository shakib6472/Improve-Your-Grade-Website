...
if ( ! class_exists( 'WPSC_CF_Textarea' ) ) :

	final class WPSC_CF_Textarea {
		 ...
		 * @var string
		 */
		public static $slug = 'cf_textarea';
		 ...
		 * @var string
		 */
		public static $data_type = 'LONGTEXT NULL DEFAULT NULL';
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 5 );
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Textarea', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		...
		public static function print_tff( $cf, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				<textarea 
					...
					autocomplete="off"><?php echo esc_attr( $val ); ?></textarea>
			</div>
			<?php
			return ob_get_clean();
		}
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					if (
						...
						wp_send_json_error( new WP_Error( 'WPSC_CF_Textarea', 'Character limit exceed!!' ), 400 );
					}
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					$value = sanitize_textarea_field( $request->get_param( $cf->slug ) );
		...
		public static function print_cf_input( $cf, $value = '' ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<textarea 
					...
					placeholder="<?php echo esc_attr( $cf->placeholder_text ); ?>"
					maxlength="<?php echo $cf->char_limit ? intval( $cf->char_limit ) : ''; ?>" 
					autocomplete="off"><?php echo esc_attr( $value ); ?></textarea>
			</div>
			<?php
		}
		...
		public static function get_cf_input_val( $cf ) {
			...
			return isset( $_POST[ $cf->slug ] ) ? sanitize_textarea_field( wp_unslash( $_POST[ $cf->slug ] ) ) : ''; // phpcs:ignore
		}
		...
		public static function print_edit_ticket_cf( $cf, $ticket ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<textarea 
					...
					placeholder="<?php echo esc_attr( $cf->placeholder_text ); ?>"
					maxlength="<?php echo $cf->char_limit ? intval( $cf->char_limit ) : ''; ?>" 
					autocomplete="off"><?php echo esc_attr( $ticket->{$cf->slug} ); ?></textarea>
			</div>
			<?php
		}
		...
		public static function set_edit_ticket_cf( $cf, $ticket ) {
			...
			$new  = isset( $_POST[ $cf->slug ] ) ? sanitize_textarea_field( wp_unslash( $_POST[ $cf->slug ] ) ) : ''; // phpcs:ignore
		...
		public static function set_rest_edit_ticket_cf( $ticket, $cf, $value ) {
			...
			if ( $ticket->{$cf->slug} != $value ) {
				$ticket->{$cf->slug} = $value;
			}
		}
		...
		public static function insert_ticket_log( $cf, $prev, $new, $current_date, $customer_id ) {
			...
			$prev_val = htmlspecialchars( $prev->{$cf->slug}, ENT_QUOTES );
			$new_val  = htmlspecialchars( $new->{$cf->slug}, ENT_QUOTES );
		...
		public static function print_edit_customer_info( $cf, $customer, $tff ) {
			...
			$val = $customer->id && $customer->{$cf->slug} ? stripslashes( htmlspecialchars( $customer->{$cf->slug} ) ) : '';
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="textarea" data-required="false" class="wpsc-input-group default_value">
					...
					<textarea name="default_value" autocomplete="off"></textarea>
				</div>
				<?php
			endif;
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="textarea" data-required="false" class="wpsc-input-group default_value">
					...
					<textarea name="default_value" autocomplete="off"><?php echo $default_val ? esc_attr( $default_val ) : ''; ?></textarea>
				</div>
				<?php
			endif;
		...
		public static function set_cf_properties( $cf, $field_class ) {
			...
			// default value.
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) {
				$default_value     = isset( $_POST['default_value'] ) ? sanitize_textarea_field( wp_unslash( $_POST['default_value'] ) ) : ''; // phpcs:ignore
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_textarea', $ticket->{$cf->slug}, $cf, $ticket, $module );
		}
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			...
			<div class="<?php echo esc_attr( $unique_id ); ?>">
				<?php echo wp_kses_post( str_replace( PHP_EOL, '<br/>', esc_html( $val ) ) ); ?>
			</div>
		...
		public static function print_val( $cf, $val ) {
			...
			echo wp_kses_post( str_replace( PHP_EOL, '<br/>', esc_html( $val ) ) );
		}
		...
		public static function get_history_log_val( $cf, $val ) {
			...
			return $val ? str_replace( PHP_EOL, '<br/>', esc_html( $val ) ) : esc_attr__( 'None', 'supportcandy' );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $ta_value );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			return $val ? wp_kses_post( str_replace( PHP_EOL, '<br/>', esc_html( $val ) ) ) : esc_attr__( 'None', 'supportcandy' );
		}
	}
endif;

WPSC_CF_Textarea::init();
