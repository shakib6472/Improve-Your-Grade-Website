...
if ( ! class_exists( 'WPSC_CF_Single_Select' ) ) :
	...
	final class WPSC_CF_Single_Select {
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			if ( $body->new ) {
				$ss_value = new WPSC_Option( $body->new );
				$value = $ss_value->name;
			} else {
				$value = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $value );
		}
