...
if ( ! class_exists( 'WPSC_CF_Email' ) ) :

	final class WPSC_CF_Email {
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 9 );
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Email Address', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					$email = WPSC_Functions::sanitize_email( sanitize_text_field( $request->get_param( $cf->slug ) ) );

					if ( in_array( $cf->field, array( 'ticket', 'agentonly' ) ) ) {

						$data[ $cf->slug ] = $cf->field == 'ticket' && $email ? $email : self::get_default_value( $cf );
						...
						if ( $email && $email != $existing_val ) {

							$customer->{$cf->slug} = $email;
							$customer->save();
		...
		public static function set_rest_edit_ticket_cf( $ticket, $cf, $value ) {
			...
			$email = WPSC_Functions::sanitize_email( sanitize_text_field( $value ) );
			if ( $ticket->{$cf->slug} != $email ) {
				$ticket->{$cf->slug} = $email;
			}
		}
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				?>
				<div data-type="email" data-required="false" class="wpsc-input-group default_value">
					<div class="label-container">
						<label for="">
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_email', $ticket->{$cf->slug}, $cf, $ticket, $module );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $email );
		}
