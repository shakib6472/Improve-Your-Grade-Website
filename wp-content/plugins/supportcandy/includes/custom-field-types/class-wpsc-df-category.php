...
if ( ! class_exists( 'WPSC_DF_Category' ) ) :

	final class WPSC_DF_Category {
		 ...
		 * @var string
		 */
		public static $slug = 'df_category';
		 ...
		 * @var string
		 */
		public static $ref_class = 'wpsc_category';
			...
			<div class="item conditional operand single">
				<select class="operand_val_1 <?php echo esc_attr( $unique_id ); ?>" <?php echo $is_multiple ? 'multiple' : ''; ?>>
					...
					foreach ( $categories as $category ) {
						$selected = '';
						if ( isset( $filter['operand_val_1'] ) && ( ( $is_multiple && in_array( $category->id, $filter['operand_val_1'] ) ) || ( ! $is_multiple && $category->id == $filter['operand_val_1'] ) ) ) {
		 ...
		 * @param WPSC_Custom_Field $cf - custom field object.
		 * @param mixed             $category - value to comapre.
		 * @return boolean
		 */
		public static function is_valid( $condition, $cf, $category ) {

			$response = false;
			switch ( $condition['operator'] ) {

				case '=':
					$response = $condition['operand_val_1'] == $category ? true : false;
					break;

				case 'IN':
					$response = in_array( $category, $condition['operand_val_1'] ) ? true : false;
					break;
		...
		public static function print_tff( $cf, $tff ) {
			...
			$val = apply_filters( 'wpsc_default_value_category', $val, $cf );

			$categories  = WPSC_Category::find( array( 'items_per_page' => 0 ) )['results'];
			$cf          = WPSC_Custom_Field::get_cf_by_slug( 'category' );
			$unique_id   = uniqid( 'wpsc_' );
			$is_disabled = apply_filters( 'wpsc_category_field_disable', false, $cf, $tff );
			ob_start();
			?>
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>" onchange="wpsc_check_tff_visibility()" <?php echo $is_disabled ? 'disabled' : ''; ?>>
					<option value=""></option>
					<?php
					foreach ( $categories as $category ) {
						?>
						<option <?php selected( $category->id, $val ); ?> value="<?php echo esc_attr( $category->id ); ?>"><?php echo esc_attr( $category->name ); ?></option>
						<?php
					}
					?>
				</select>

				<?php do_action( 'wpsc_after_print_tff_category', $val, $cf ); ?>
		...
		public static function set_create_ticket_data( $data, $custom_fields, $is_my_profile ) {
			...
			$value            = self::get_tff_value( 'category' );
			$data['category'] = $value ? $value : self::get_default_value( $custom_fields[ self::$slug ][0] );
			return $data;
		}
		...
		public static function set_rest_ticket_data( $data, $request, $custom_fields ) {
			...
			$cf = WPSC_Custom_Field::get_cf_by_slug( 'category' );
			$category = new WPSC_Category( intval( $request->get_param( 'category' ) ) );
			$data['category'] = $category->id ? $category->id : self::get_default_value( $cf );
			return $data;
		}
		 ...
		 *
		 * @param array $joins - join strings in an array which later will be imploded.
		...
		public static function ticket_join( $joins, $filter ) {
			...
			global $wpdb;
			if ( $filter['orderby_slug'] == 'category' ) {
				$joins[] = 'LEFT JOIN ' . $wpdb->prefix . 'psmsc_categories ct ON t.category = ct.id';
			}
			return $joins;
		}
		 ...
		 *
		 * @param WPSC_Custom_Field $cf - custom field object.
		 * @param WPSC_Category     $value - optional. Input field will be printed with given value (if any).
		 * @return void
		 */
		public static function print_cf_input( $cf, $value = '' ) {
			...
			if ( ! is_object( $value ) ) {
				...
				$category = $value ? new WPSC_Category( $value ) : 0;
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>">
					...
					foreach ( $categories as $category ) {
						?>
						<option <?php $value && selected( $category->id, $value->id ); ?> value="<?php echo esc_attr( $category->id ); ?>"><?php echo esc_attr( $category->name ); ?></option>
						<?php
					}
					?>
				</select>
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="single-select" data-required="false" class="wpsc-input-group default_value">
					...
					<select id="wpsc-default-tc" name="default_value">
						<?php
						$categories = WPSC_Category::find( array( 'items_per_page' => 0 ) )['results'];
						foreach ( $categories as $category ) {
							?>
							<option <?php echo selected( $category->id, $default_val ); ?> value="<?php echo esc_attr( $category->id ); ?>"><?php echo esc_attr( $category->name ); ?></option>
							<?php
						}
						?>
					</select>
					<script>
						jQuery('#wpsc-default-tc').selectWoo({
							allowClear: true,
							placeholder: ""
						});
					</script>
		...
		public static function get_orderby_string( $cf ) {
			...
			return 'FIELD( t.category, ' . $categories . ' )';
		}
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_category', $ticket->category->name, $cf, $ticket, $module );
		}
		...
		public static function print_tl_ticket_field_val( $cf, $ticket ) {
			...
			echo esc_attr( $ticket->category->name );
		}
		...
		public static function print_val( $cf, $val ) {
			...
			$category = is_object( $val ) ? $val : new WPSC_Category( $val );
			echo $category->id ? esc_attr( $category->name ) : esc_attr__( 'None', 'supportcandy' );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			$new_category = new WPSC_Category( $body->new );
			if ( $new_category->id ) {
				$category_value = $new_category->name;
			} else {
				$category_value = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the ' . esc_attr( $cf->name ) . ' of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $category_value );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$category = is_object( $val ) ? $val : new WPSC_Category( $val );
			return $category->id ? esc_attr( $category->name ) : esc_attr__( 'None', 'supportcandy' );
		}
	}
endif;

WPSC_DF_Category::init();
