...
if ( ! class_exists( 'WPSC_CF_Radio_Button' ) ) :

	final class WPSC_CF_Radio_Button {
		 ...
		 * @var string
		 */
		public static $slug = 'cf_radio_button';
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 3 );
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Radio Button', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				foreach ( $options as $option ) {
					...
					<div class="radio-container">
						...
						<input id="<?php echo esc_attr( $unique_id ); ?>" type="radio" <?php echo esc_attr( $checked ); ?> name="<?php echo esc_attr( $cf->slug ); ?>" onchange="wpsc_check_tff_visibility()" value="<?php echo esc_attr( $option->id ); ?>"/>
		...
		public static function print_cf_input( $cf, $value = '' ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<?php
				foreach ( $options as $option ) :
					$checked = $value && $option->id == $value->id ? 'checked' : ''
					?>
					<div class="radio-container">
						<?php $unique_id = uniqid( 'wpsc_' ); ?>
						<input id="<?php echo esc_attr( $unique_id ); ?>" type="radio" <?php echo esc_attr( $checked ); ?> name="<?php echo esc_attr( $cf->slug ); ?>" value="<?php echo esc_attr( $option->id ); ?>"/>
						<label for="<?php echo esc_attr( $unique_id ); ?>"><?php echo esc_attr( $option->name ); ?></label>
					</div>
					<?php
				endforeach;
		...
		public static function print_edit_ticket_cf( $cf, $ticket ) {
			...
			<div class="wpsc-tff wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible wpsc-xs-12" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<?php
				foreach ( $options as $option ) :
					$checked = $ticket->{$cf->slug} && $option->id == $ticket->{$cf->slug}->id ? 'checked' : ''
					?>
					<div class="radio-container">
						<?php $unique_id = uniqid( 'wpsc_' ); ?>
						<input id="<?php echo esc_attr( $unique_id ); ?>" type="radio" <?php echo esc_attr( $checked ); ?> name="<?php echo esc_attr( $cf->slug ); ?>" value="<?php echo esc_attr( $option->id ); ?>"/>
		...
		public static function edit_cf_setting_body( $cf, $type ) {
			...
			if ( in_array( $cf->field, array( 'ticket', 'agentonly' ) ) ) :
				?>
				<div class="wpsc-input-group conditional default-val-option-single">
					<div class="label-container">
						<label for="">
					...
					<select name="default-val-option-single[]" id="wpsc-default-val-option-single">
						...
						<?php
						foreach ( $options as $option ) :
							echo '<option ' . selected( $cf->default_value[0], $option->id ) . ' value=' . esc_attr( $option->id ) . '>' . esc_attr( $option->name ) . '</option>';
						endforeach;
						?>
					</select>
				</div>
		...
		public static function ticket_join( $joins, $filter ) {
			...
			if ( $cf && $cf->type::$slug == 'cf_radio_button' ) {
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_radio_button', $value, $cf, $ticket, $module );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			if ( $body->new ) {
				$value = new WPSC_Option( $body->new );
				$rb_value = $value->name;
			} else {
				$rb_value = 'None';
			}
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $rb_value );
		}
		...
		public static function get_field_value( $cf, $val ) {
			...
			$option = is_object( $val ) ? $val : new WPSC_Option( $val );
			return $option->id ? esc_attr( $option->name ) : esc_attr__( 'None', 'supportcandy' );
		}
	}
endif;

WPSC_CF_Radio_Button::init();
