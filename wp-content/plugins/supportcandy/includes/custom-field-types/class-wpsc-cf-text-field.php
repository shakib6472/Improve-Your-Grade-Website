...
if ( ! class_exists( 'WPSC_CF_Text_Field' ) ) :

	final class WPSC_CF_Text_Field {
		...
		public static function init() {
			...
			add_filter( 'wpsc_cf_types', array( __CLASS__, 'add_cf_type' ), 1 );
		...
		public static function add_cf_type( $cf_types ) {
			...
			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'Textfield', 'supportcandy' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		...
		public static function print_tff( $cf, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>" data-slug="<?php echo esc_attr( $cf->slug ); ?>">
				...
				<input 
					...
					maxlength="<?php echo $cf->char_limit && $cf->char_limit < 255 ? intval( $cf->char_limit ) : '255'; ?>" 
		...
		public static function set_create_ticket_data( $data, $custom_fields, $is_my_profile ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					$limit = $cf->char_limit && $cf->char_limit < 255 ? $cf->char_limit : 255;
					$value = substr( $value, 0, $limit );

					if (
						$cf->char_limit && mb_strlen( $value ) > $cf->char_limit
					) {
						wp_send_json_error( new WP_Error( 'WPSC_CF_Text_Field', 'Character limit exceed!!' ), 400 );
					}
		...
		public static function get_cf_input_val( $cf ) {
			...
			return isset( $_POST[ $cf->slug ] ) ? sanitize_text_field( wp_unslash( $_POST[ $cf->slug ] ) ) : ''; // phpcs:ignore
		}
		...
		public static function set_edit_ticket_cf( $cf, $ticket ) {
			...
			$char_limit = $cf->char_limit && $cf->char_limit < 255 ? $cf->char_limit : 255;
			$new = substr( $new, 0, $char_limit );
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			return apply_filters( 'wpsc_ticket_field_val_text_field', $ticket->{$cf->slug}, $cf, $ticket, $module );
		}
		...
		public static function print_val( $cf, $val ) {
			...
			echo $val ? esc_attr( $val ) : esc_attr__( 'None', 'supportcandy' );
		}
		...
		public static function print_activity( $cf, $recent_logs, $body, $view ) {
			...
			return esc_attr( $recent_logs->customer->name ) . ' updated the' . esc_attr( $cf->name ) . ' value of <a href="' . esc_attr( $url ) . '" target="_blank">#' . esc_attr( $recent_logs->ticket->id ) . '</a> to ' . esc_attr( $tf_value );
		}
