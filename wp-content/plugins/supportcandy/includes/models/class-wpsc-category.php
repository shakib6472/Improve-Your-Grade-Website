...
if ( ! class_exists( 'WPSC_Category' ) ) :
	...
	final class WPSC_Category {
		...
		public static function apply_schema() {
			...
			self::$schema = apply_filters( 'wpsc_category_schema', $schema );

			// Prevent modify.
			$prevent_modify       = array( 'id' );
			self::$prevent_modify = apply_filters( 'wpsc_category_prevent_modify', $prevent_modify );
		}
		...
		public function __construct( $id = 0 ) {
			...
			if ( $id > 0 ) {
				...
				$category = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}psmsc_categories WHERE id = " . $id, ARRAY_A );
				...
				foreach ( $category as $key => $val ) {
					...
					if ( $key == 'name' ) {
						...
						$this->data[ $key ] = $val !== null ? WPSC_Translations::get( 'wpsc-category-' . $category['id'], stripslashes( $val ) ) : '';
		...
		public function save() {
			...
			if ( ! isset( $data['id'] ) ) {
				...
				WPSC_Translations::add( 'wpsc-category-' . $this->data['id'], $this->data['name'] );
			}
		...
		public static function insert( $data ) {
			...
			if ( ! isset( $data['load_order'] ) ) {
				$max_order          = $wpdb->get_var( "SELECT MAX(load_order) FROM {$wpdb->prefix}psmsc_categories" );
			...
			// string translation.
			WPSC_Translations::add( 'wpsc-category-' . $id, $data['name'] );
		 ...
		 *
		 * @param WPSC_Category $category - category object.
		 * @return boolean
		 */
		public static function destroy( $category ) {
			...
			WPSC_Translations::remove( 'wpsc-category-' . $category->id );
		...
		private function set_data( $data ) {
			...
			foreach ( $data as $var_name => $val ) {
				...
				if ( $var_name == 'name' ) {
					...
					$this->data[ $var_name ] = $val !== null ? WPSC_Translations::get( 'wpsc-category-' . $data['id'], stripslashes( $val ) ) : '';
		...
		public static function find( $filter = array(), $is_object = true ) {
			...
			$sql   = 'SELECT * FROM ' . $wpdb->prefix . 'psmsc_categories ';
			...
			// total results.
			$sql   = 'SELECT count(id) FROM ' . $wpdb->prefix . 'psmsc_categories ';
		...
		private static function get_where( $filter ) {
			...
			$meta_query = apply_filters( 'wpsc_category_meta_query', $meta_query, $filter );
			if ( $meta_query ) {
