...
if ( ! class_exists( 'WPSC_EN_Reply_Ticket' ) ) :

	final class WPSC_EN_Reply_Ticket extends WPSC_Email_Notifications {
		 ...
		 * @var string
		 */
		private static $slug = 'reply-ticket';
		...
		public static function init() {
			...
			// process event.
			add_action( 'wpsc_post_reply', array( __CLASS__, 'process_event' ), 200 );
		}
		...
		public static function process_event( $thread ) {
			...
			foreach ( $email_templates as $key => $et ) {
				...
				$data = apply_filters( 'wpsc_reply_ticket_email_data', $data, $en );

				WPSC_Background_Email::insert( $data );
			}
		}
	}
endif;

WPSC_EN_Reply_Ticket::init();
