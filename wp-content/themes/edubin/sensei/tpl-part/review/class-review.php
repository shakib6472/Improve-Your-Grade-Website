...
class Edubin_SS_Course_Review {
	...
	public function comments_template_locator( $template ) {
	    if ( self::POST_TYPE === get_post_type() ) :
	    	return get_template_directory() . '/sensei/custom/review/review-box.php';
		endif;
	    return $template;
	}
	
	public static function ss_course_comments( $comment, $args, $depth ) {
		$GLOBALS['comment'] = $comment;
	    set_query_var( 'comment', $comment );
	    set_query_var( 'args', $args );
	    set_query_var( 'depth', $depth );
	    get_template_part( 
			'sensei/custom/review/review-list', 
			'', 
			array(
				'comment' => $comment, 
				'args'    => $args, 
	...
	public function save_rating_comment( $comment_id, $comment_approved, $data ) {
	    ...
	    if ( self::POST_TYPE === $post_type && isset( $_POST['rating'] ) ) :
	        add_comment_meta( $comment_id, 'ss_rating', $_POST['rating'] );
	...
	public static function get_average_ratings( $post_id ) {
	    return get_post_meta( $post_id, '_ss_average_rating', true );
	}
	...
	public static function update_average_ratings( $post_id ) {
	    ...
	    if ( self::POST_TYPE === $post_type ) :
	        ...
	        update_post_meta( $post_id, '_ss_average_rating', $average_rating );
	        ...
	        $courses = edubin_sensei_get_courses( $args );
	        $author_average_rating = 0;

	        if ( ! empty( $courses ) ) :
	        	foreach ( $courses as $course_id ) :
	        		$average_rating = get_post_meta( $post_id, '_ss_average_rating', true );
	        		if ( ! empty( $average_rating ) && $average_rating > 0 ) :
	        			$author_average_rating += $average_rating;
					endif;
				endforeach;
			endif;
	        update_user_meta( $author_id, '_ss_average_rating', $author_average_rating );
	    endif;
	}
	...
	public static function display_review( $rate, $style = 'default' ) {
		global $post;
		$rate = $rate ? $rate : 0;
		$rating = Edubin_SS_Course_Review::get_all_reviews( get_the_ID() );
		$average_rating = Edubin_SS_Course_Review::get_average_ratings( get_the_ID() );
...
Edubin_SS_Course_Review::instance();