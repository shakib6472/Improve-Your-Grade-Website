		...
		<table class="system-status-table">
			<tbody>
				...
				<tr>
					...
					if ( $memory < 268435456 ) {
						echo '<mark class="error">' . sprintf(__('%s - We recommend setting memory to at least 256MB. To increase it watch the %s.','edubin'), size_format( $memory ), '<a href="' . esc_url('https://www.youtube.com/watch?v=PZDeLVSXSu0') . '" target="_blank">'.esc_html__('video','edubin').'</a>') . '</mark>';
					} else {
				...
				<tr>
					...
					if ( $max_input < 3000 ) {
						echo '<mark class="error">' . sprintf( wp_kses(__( '%s - We recommend setting PHP max_input_vars to at least 3000. To increase it watch the <a href="%s" target="_blank">video</a>', 'edubin' ), array( 'a' => array( 'href' => array(),'target' => array() ) ) ), $max_input, 'https://www.youtube.com/watch?v=NJXzKDnyJ-4' ) . '</mark>';
					} else {
